...
func (s *Session) RequestWithLockedBucket(method, urlStr, contentType string, b []byte, bucket *Bucket, sequence int) (response []byte, err error) {
	...
	err = s.Client.DoTimeout(req, res, 20*time.Second)

	if err != nil {
		fasthttp.ReleaseRequest(req)
		fasthttp.ReleaseResponse(res)
		bucket.Release(fasthttp.ResponseHeader{})
		return
	}
	...
	case 502:
		...
		if sequence < s.MaxRestRetries {
			...
			s.log(LogInformational, "%s Failed (%s), Retrying...", urlStr, res.StatusCode())
			response, err = s.RequestWithLockedBucket(method, urlStr, contentType, b, s.Ratelimiter.LockBucketObject(bucket), sequence+1)
		} else {
			err = fmt.Errorf("Exceeded Max retries HTTP %s, %s", res.StatusCode(), response)
		}
	...
	default: // Error condition
		err = newRestError(req, res, response)
	}
